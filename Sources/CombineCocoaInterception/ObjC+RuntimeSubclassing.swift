    ...
    synchronized(subclass) {
      ...
      if !subclassAssociations.value(forKey: hasSwizzledKey) {
        ...
        for (selector, body) in pairs {
          ...
          if method_getImplementation(method) == _combinecocoa_objc_msgForward {
            let succeeds = class_addMethod(
              subclass,
              selector.interopAlias,
            ...
            precondition(
              succeeds,
              "RAC attempts to swizzle a selector that has already a runtime injected implementation. This is unsupported in the current version."
            )
          }
        }
      }
