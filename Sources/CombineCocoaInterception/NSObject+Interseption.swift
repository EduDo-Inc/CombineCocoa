...
extension NSObject {
  ...
  ///
  /// - note: Observers to the resulting publisher should not call the method
      ...
      synchronized(subclass) {
        ...
        if !class_respondsToSelector(subclass, interopAlias) {
          let immediateImpl = class_getImmediateMethod(subclass, selector)
            ...
            .flatMap { $0 != _combinecocoa_objc_msgForward ? $0 : nil }

          if let impl = immediateImpl {
            let succeeds = class_addMethod(subclass, interopAlias, impl, typeEncoding)
            precondition(
              succeeds,
              "RAC attempts to swizzle a selector that has message forwarding enabled with a runtime injected implementation. This is unsupported in the current version."
            )
          }
        }
      }

      let state = InterceptingState()
      associations.setValue(state, forKey: stateKey)

      // Start forwarding the messages of the selector.
      _ = class_replaceMethod(subclass, selector, _combinecocoa_objc_msgForward, typeEncoding)
...
private func enableMessageForwarding(_ realClass: AnyClass, _ selectorCache: SelectorCache) {
  ...
  let newForwardInvocation: ForwardInvocationImpl = { objectRef, invocation in
    ...
    let impl: IMP = method.map(method_getImplementation) ?? _combinecocoa_objc_msgForward
    if impl != _combinecocoa_objc_msgForward {
